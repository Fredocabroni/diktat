...
- **Privy backfill for pre-090009 signups** — 4 human dev-DB accounts (created 2026-04-22, 2026-05-03) predate migration `090009`'s `handle_new_user` privy_provision rewire and have `wallets.privy_user_id` null. When `PRIVY_ENABLED` flips true, the per-signup `pg_notify('privy_provision')` only fires for fresh `auth.users` rows — these 4 won't auto-provision. Backfill once: `select pg_notify('privy_provision', w.user_id::text) from public.wallets w join public.users u on u.id = w.user_id where w.privy_user_id is null and u.is_bot = false;` (or call `buildPrivyProvider` directly in a one-shot script). Trivial volume; do it before exercising wallet flows with those accounts.
...
- **`.env.local` secrets go stale across machine / time boundaries.** This session lost two days to credential rot — the Upstash REST token was treated-as-compromised after being shared in chat (existing TODO), and during Phase 4 setup the `DATABASE_URL` password failed SASL auth and had to be reset via the Supabase dashboard. **On resume (new machine, new branch, or after >1 week), verify and rotate first:** test `DATABASE_URL` against the session-pooler (`postgres.<ref>` user, port 5432) and the `UPSTASH_REDIS_REST_TOKEN` against the Upstash REST URL before running any dev work that needs them. A 30-second smoke test up front is cheaper than discovering it mid-debug.
- **`supabase db push` ignores the password in `--db-url`.** `migration list` and `migration repair` honor the full URL (incl. password) for auth, but `db push` reads `SUPABASE_DB_PASSWORD` from env and rejects whatever password is in the URL — yielding a confusing "password authentication failed for user 'postgres'" on a credential that just worked moments before in the same session. Set `SUPABASE_DB_PASSWORD` in the subshell before pushing: `(export SUPABASE_DB_PASSWORD=$(printf '%s' "$DB_URL" | python3 -c 'import sys,urllib.parse; print(urllib.parse.unquote(urllib.parse.urlparse(sys.stdin.read().strip()).password), end="")'); supabase db push --db-url "$DB_URL" --dry-run)`.
- **Use full 14-char timestamps for new migrations.** The 8-char date-only names (`20260427`, `20260522`, `20260523`) trip the supabase CLI's local-vs-remote version pairing — `migration list` will show the same version twice (once Local-only, once Remote-only) even when the bookkeeping is correct. `db push` resolves them fine, but the cosmetic confusion is avoidable: name new migrations `YYYYMMDDHHMMSS_<name>.sql` (e.g. `20260524130000_<name>.sql`), not date-only.
- **Open-debate `scoreAndSettle` re-entry is not crash-safe for AI-tiebroken debates.** Belongs with the orphan-battle-recovery deferral noted in the "Phase 3 queue mechanism (no BullMQ — Path B)" bullet 4 above. If the workers process crashes between `updateRoundPayload` (verdict written) and `markBattleSettled` (battle marked `settled`), the next runner pass re-enters `scoreAndSettle` because `battles.status` is still `'live'`. For community-decided debates this is benign — the tally is deterministic and `apply_ap_drafts` is idempotent on `(battle_id, user_id, reason)` keys. For **AI-tiebroken** debates (community AP exactly even) the re-called AI can flip `winnerSeat`; the new winner/loser drafts use the same `(battle, user, 'battle_win'|'battle_loss')` idempotency keys as the prior attempt, so the role-flip writes **conflicting drafts that bypass idempotency** → corrupted AP for that debate. **Fix when crash-recovery lands** (BullMQ-TCP migration or interim orphan-battle reaper): on re-entry, if the verdict round is already `state='scored'`, read `winner_user_id` directly from the verdict row and jump to `markBattleSettled` — never re-tally, never re-call AI.
