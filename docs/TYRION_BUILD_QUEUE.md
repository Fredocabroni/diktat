...
- **Drop UI follow-ups (from PR 4.2 reviewer asks).** Three deferred items:
  1. **`feed.list` cursor future-clamp** (`apps/api/src/routers/feed.ts:96`) — `cursor` is `z.string().datetime()`-validated but not clamped to `<= now`. A client passing `cursor='9999-12-31T23:59:59Z'` would return any future-scheduled Drop row the moment `drop_publish` lands it, before `drop_at` arrives. The pipeline today stamps `drop_at` at insert time (not ahead-of-time), so this is theoretical — but the same future-clamp follow-up is already on the queue for `wallet.transactions`, and the pattern should apply here for the same reason. Fix shape: `const cursorMs = Math.min(Date.parse(input?.cursor ?? new Date().toISOString()), Date.now()); ... .lte('drop_at', new Date(cursorMs).toISOString())`. One line. PR 4.2 security-reviewer LOW finding #2.
  2. **`feed.list` rate-limit registration** (`apps/api`) — `feed.list` is a `protectedProcedure`, but `@fastify/rate-limit` isn't registered yet (existing CLAUDE.md TODO). With PR 4.2 live, every signed-in client polls `feed.list` on every navigation. Before the API gets a public DNS, add this endpoint to the documented rate-limit set (suggest 30 req/min per `ctx.userId`). Not a regression — one more endpoint on the existing pre-launch rate-limit TODO. PR 4.2 security-reviewer LOW finding #3.
