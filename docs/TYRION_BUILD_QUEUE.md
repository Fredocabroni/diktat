...
- **[P0-before-prod] Reviewer workflows pass silently on agent-side credit/API errors — gate-integrity bug.** PR #48 surfaced it: all five `claude-review` jobs (schema-reviewer, security-reviewer, copy-linter, addiction-auditor, claude-migrate-review) exit 0 and post a literal `"Credit balance is too low"` comment when the Anthropic API key on the GHA runner runs out of credit, instead of failing the job. CI rolls up SUCCESS; the merge gate reads green; the LLM review never actually ran. Same failure shape will repeat for any agent-side error the workflows currently swallow (rate limits, network errors, malformed responses, API outages). **Risk vector:** sibling to the migration-apply gap — a silently-green review gate is worse than a clearly-failed one. Today the deterministic checks (unit tests + probe + Vercel preview + tsc) still hold, but the architectural promise that "schema-reviewer + security-reviewer approve every migration before merge" is broken whenever credits run dry. **Fix:** in `.github/workflows/claude-review.yml` and `.github/workflows/claude-migrate-review.yml`, detect known agent-error response shapes (`Credit balance is too low`, `rate_limit_error`, `overloaded_error`, `internal_server_error`, empty body, etc.) and `exit 1` the step so the check goes RED. The comment body should still post for visibility — the difference is the conclusion. Pair with the credit-balance alert entry below so the failure mode is rare. Must land before any production-scale review traffic — silent-green reviewers on a load-bearing PR is a wrong-color-merge bug.
