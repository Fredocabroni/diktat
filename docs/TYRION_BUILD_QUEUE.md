...
- **Pre-launch — `neutrality-auditor` subagent MUST land before the fact-check surface goes public.** PR 4.7 shipped the fact-check orchestrator with the integrity contract in `packages/ai-fabric/src/prompts/fact-check.ts` + the `FactCheckResultSchema` verdict enum + the `retrieval_mode` field. Today's CI pipeline gates the prompt via `copy-linter` + a manual neutrality eyeball at PR time. That covers the introduction. The risk it does NOT cover is an **ungated edit six months out** that quietly removes the "always `contested` for value judgments" rule, softens the empirical-disagreement steering ("never manufacture confidence" when credible sources disagree), or weakens the projections-vs-raw-data distinction. Scope a `neutrality-auditor` subagent that runs automatically on any change to: (a) `packages/ai-fabric/src/prompts/fact-check.ts`, (b) the `FactCheckResultSchema` verdict enum or `retrieval_mode` enum in `packages/ai-fabric/src/types.ts`, (c) the SQL CHECK on `fact_check_verdicts.verdict` and the row-level check on `contested_reason`, (d) any user-facing copy that renders a verdict label. Must-land-before any code path that surfaces fact-check verdicts to the public. Without it, the integrity contract is only as strong as whoever's reviewing the next PR.
- **Trivia battle-runner crash-window — AUDITED 2026-06-15; same shape as open-debate's pre-#33 vulnerability, but inverted write order + one-shot (not tick-based) lifecycle means the fix shape diverges. Bundle with the Phase 3.5 reaper PR.** `apps/workers/src/jobs/battle-runner.ts` `settle()` (L329–421) flips `battles.status='settled'` FIRST (L374) and applies AP drafts LAST (L411) — the inverse of open-debate. A worker crash between those two writes leaves the battle `status='settled'` with no AP applied; the existing poller scans on `status='live'` only, so the window is invisible to current and planned (Phase 3.5 reaper) re-entry mechanisms. `runBattle` is a single async sequence — no `runTriviaBattleTick` equivalent to guard at the top, so the #33 "guard on entry to next tick" pattern doesn't transplant. Dormant today (no re-entry mechanism + no public traffic), but the structural gap is real, and `apply_ap_drafts` idempotency alone doesn't catch drift: a reaper that re-fetches participants on resume would see `current_ap`/`tier_id` drift between first-pass and reaper-pass, producing split-brain ledger entries on a partial first-pass (winner credited at original delta via idempotency short-circuit, loser debited at drifted delta or vice versa). Fix bundles three coupled pieces with the Phase 3.5 reaper PR: (1) reorder writes in `settle()` so apply-drafts runs first and `status='settled'` flips last (matches open-debate; converts crash-window into a discoverable `status='live'` state); (2) stamp a `_settlement_inputs` snapshot (`{winner, loser}` × `{apBefore, tier, consecutiveLosses, reductionsUsed}`) into a durable target (new `battles.pending_settlement_inputs jsonb` column OR a synthetic `battle_rounds` settle-marker at `round_no=ROUND_COUNT` — pick at reaper-PR design time); (3) the reaper itself, which re-runs `settle()` reading the snapshot to produce byte-identical drafts. Test surface mirrors #33's set: snapshot non-regression (TEST 5 analog), Window A — `applyDrafts` transient failure two-tick recovery (TEST 4 analog), Window B — `markSettled` failure after AP succeeds two-tick recovery (TEST 6 analog), drift-resistance — mutate `users.current_ap`/`tier_id` between passes and assert drafts are byte-identical (TEST 2 analog; no AI-tiebreaker equivalent — trivia scoring is fully deterministic on immutable `trivia_answers`). Landing the pieces piecemeal creates worse intermediate states: reorder alone leaves stuck `live` battles with no settler; snapshot alone has no consumer; reaper alone (current plan) re-fetches live participants and produces drift.
