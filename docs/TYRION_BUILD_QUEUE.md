...
- **[HIGH] H2 — `wallet.transactions` cursor has no upper-bound clamp.** Cross-reference: pre-existing CLAUDE.md TODO ("API cursor clamp + aggregate push-down"). PR #41 grants SELECT on `ap_transactions` so the router now reaches the cursor logic at runtime; before this PR it would have 42501'd at PostgREST and the cursor was unreachable. Fix shape: `.refine(v => !v || new Date(v) <= new Date(), ...)` on the Zod cursor + `.lte('created_at', new Date().toISOString())` server-side. Same pattern as the cursor clamp `feed.list` ships. Must land before public launch.

- **[HIGH] H3 — `wallet.ghostEarnings` is an unbounded client-side aggregate.** Cross-reference: pre-existing CLAUDE.md TODO ("aggregate push-down"). Same reachability story as H2 — PR #41's grant on `ap_transactions` makes the unbounded `select('delta')` + Node-side sum live. Push to PostgREST aggregate (`select('delta.sum()')`) or a SECURITY INVOKER SQL function. Must land before any user can accumulate >10k transactions.

- **[MEDIUM] M3 — `sessions` UPDATE policy missing `WITH CHECK`. ACTIVATED BY 20260617160000, live on dev now.** The UPDATE policy is `using (is_self(user_id) and ended_at is null)` with no `WITH CHECK`. A self-update can now arbitrarily set `started_at` to a past date or `ended_at` to a far-future date — no maximum session length is enforced at the DB layer. Fix: `with check (public.is_self(user_id) and ended_at is not null and ended_at <= now() + interval '1 minute')`. Closes the column-rewrite vector entirely; a self-update can only close a session, not extend or rewrite it. Must land before public launch.

- **[MEDIUM] M4 — `trivia_questions.correct_index` is readable by every authenticated user. ACTIVATED BY 20260617160000, live on dev now.** The `trivia_q_select_verified` policy returns the full row; any authenticated PostgREST call to `/rest/v1/trivia_questions?select=*&verified=eq.true` returns the answer key for all 184 verified questions on dev. The trivia battle surface is a cheat-sheet today. Fix: column-level `grant select (id, prompt, category, choices, difficulty, source_url, verified)` on `trivia_questions` — drop `correct_index` from the grant. Move answer-grading server-side to a `SECURITY DEFINER` RPC that returns `{ correct: boolean }` without exposing the index. Must land before any trivia surface goes public.

- **[MEDIUM] M5 — `factCheck.enqueue` has no rate limit; each call triggers an AI worker invocation.** `@fastify/rate-limit` is not registered (existing CLAUDE.md TODO). PR #41 doesn't make this worse, but the security-reviewer surfaced it alongside the others so the public-launch checklist is authoritative in one place. Suggested initial budget: 20 enqueues/day per `ctx.userId`, Redis counter + TTL. The highest-priority rate-limit slot because the cost vector is AI-spend per call, not just DB load. Must land before the fact-check surface is publicly reachable.
...
- **Systemic GRANT gap on `public` schema for client roles (pre-launch BLOCKER, discovered during PR #40 round-3 RLS investigation).** The dev DB has **zero** `GRANT SELECT` (or any other privilege) to `anon` or `authenticated` on any table in `public`. Only `service_role` has grants (via migration `20260427_grant_service_role_public.sql`). A forged `role: authenticated` JWT hitting `/rest/v1/news_topics`, `/rest/v1/wallets`, or any other table returns `42501 permission denied for table <name>` before RLS is consulted. Every `news_topics_select_all`-style policy across our migrations is effectively dead-letter — the policy says "read-all" but the table-privilege layer denies the call. **Impact:** every `ctx.db.from(...)` read in `apps/api/src/routers/*.ts` (which all use `userScopedClient` = anon-key + user bearer) routes through PostgREST as `authenticated` and would 42501 in production. Unit tests pass because they use `fakeDb` (mock); the runtime path has never been exercised against real PostgREST from a logged-in client. **Remediation options:** (a) ship a sibling to `20260427_grant_service_role_public.sql` that grants `SELECT` (and policy-gated `INSERT`/`UPDATE`/`DELETE`) on every public table to `anon` + `authenticated`, OR (b) flip every reader router to `serviceRoleClient` (overkill — defeats the RLS-as-authority design). Option (a) is the conventional Supabase shape; before merging it, audit every existing RLS policy to confirm the intended access posture. Track as a **pre-launch hard blocker** — every reader endpoint is functionally broken in production until this lands.
