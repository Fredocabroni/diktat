...
- **[fix(api): slim runtime image — reinstall --prod after build] PR #80 security-reviewer MED #2 — strip devDependencies from the runtime container.** PR #80 added `--prod=false` to the Railway `buildCommand` so `turbo` (a root devDep) is available during the build. Nixpacks does NOT multi-stage by default — install and runtime share one container — so every devDep installed for the build (`tsx`, `vitest`, `typescript`, `@types/node`, `@vitest/coverage-v8`, plus the root tooling `turbo`, `eslint`, `prettier`, `@eslint/js`, `typescript-eslint`) remains in the runtime image. Direct attack surface today is limited (lockfile is committed, `--frozen-lockfile` bounds drift, none of the present devDeps has a known critical CVE), but residual attack-surface inflation is the structural concern: a future CVE in `vitest` / `tsx` / their transitive trees becomes exploitable from inside the runtime container despite serving no runtime purpose, and `tsx` specifically lowers the bar for anyone with RCE to execute arbitrary TypeScript without needing a compile step.

  **Correct command (verified locally end-to-end):**

  ```toml
  buildCommand = "pnpm install --frozen-lockfile --prod=false && pnpm turbo build --filter=@diktat/api && pnpm install --frozen-lockfile --prod"
  ```

  The third stage re-runs `pnpm install` with `--prod` flag set, which respects the committed lockfile but excludes devDependencies from the manifest. Verified result: removes **29** devDep packages from `node_modules`, preserves all runtime deps (`@diktat/{auth,db,shared}`, `@fastify/cors`, `@supabase/supabase-js`, `@trpc/server`, `@upstash/redis`, `fastify`, `superjson`, `zod`); `apps/api/dist/server.js` survives the re-install (only `node_modules` is touched); `node dist/server.js` boots clean, `/health` returns HTTP 200.

  **DO NOT use `pnpm prune --prod` — it is broken in this monorepo.** Proven locally: `pnpm prune --prod` in a pnpm workspace **wipes `node_modules` entirely** ("The modules directories will be removed and reinstalled from scratch" is literal) rather than just stripping devDeps. Runtime then crashes immediately:

  ```
  Error [ERR_MODULE_NOT_FOUND]: Cannot find package '@fastify/cors'
    imported from /home/runner/work/diktat/diktat/apps/api/dist/server.js
  ```

  `CI=true` (which Railway sets by default) suppresses the interactive `Proceed? (Y/n)` prompt but does NOT save the runtime — it just lets the destructive prune run unattended. The MED #2 fix MUST use `pnpm install --frozen-lockfile --prod` as the third stage; the security-reviewer's literal recommendation (`pnpm prune --prod`) would brick the Railway deploy.

  **Also fold in: `declarationMap: false` in `packages/{db,shared,auth}/tsconfig.build.json` (PR #80 security-reviewer MED #1).** PR #79 added `declarationMap: true` to enable composite project references. The `.d.ts.map` files emitted to each package's `dist/` embed absolute filesystem paths to source files on the build machine (e.g. `/home/runner/work/diktat/diktat/packages/auth/src/verify.ts` — including the JWT verification source path). These files are functionally useless in the runtime container (`declarationMap` is an IDE go-to-source convenience) and disclose the full CI/runner directory layout to anyone who can introspect the deployed container. `apps/api/tsconfig.build.json` correctly already has `declarationMap: false`; the three packages should match. One-character change per file × 3 files. PR #80 security-reviewer MED #1.

  **Also fold in: CORS null-origin branch — `apps/api/src/server.ts` `cors.origin` callback (PR #82 security-reviewer MED, new).** The current callback returns `cb(null, true)` when `origin === undefined` (the `null-origin` shape), allowing CORS headers to be set on requests from `file://` pages, sandboxed iframes, opaque cross-origin fetches, or programmatic requests where the browser elides the `Origin` header. Today's blast radius is bounded — the API is bearer-only (`credentials: false`), so cookies are not at risk and CORS alone cannot exfiltrate session state. But the pattern is incorrect for any future endpoint that switches to `credentials: true` or adds cookie-based auth (e.g. a future internal admin surface, server-side OAuth callback, or a CSRF-token endpoint). **Fix shape**: replace `if (!origin) return cb(null, true)` with `if (!origin) return cb(null, false)` — the only legitimate callers are listed in `env.WEB_ORIGINS`. If a legitimate null-origin caller surfaces later, document it explicitly with a code comment naming the surface and re-evaluate the security envelope at that point. Pure structural posture; not exploitable today, structurally wrong for the trust model. PR #82 round-1 security-reviewer MED (CORS null-origin).

  **Also fold in: `SUPABASE_JWT_ISSUER` required in non-dev/test (PR #82 security-reviewer LOW, new).** `apps/api/src/env.ts` declares `SUPABASE_JWT_ISSUER: z.string().url().optional()`. In `context.ts`, the issuer claim is enforced ONLY when the env var is set (`...(env.SUPABASE_JWT_ISSUER ? { issuer: env.SUPABASE_JWT_ISSUER } : {})`). When unset, `jose`'s `jwtVerify` does not check the `iss` claim — a JWT signed with the same `SUPABASE_JWT_SECRET` but issued by a different Supabase project (e.g. a dev secret accidentally reused in prod, or a compromised sibling project on the same shared secret) would be accepted as valid. **Fix shape**: extend the boot-time activation-safety gate in `server.ts` to require `SUPABASE_JWT_ISSUER` whenever `NODE_ENV` is not `'development'` or `'test'` (same shape as the existing `TRUSTED_PROXY_HOPS` gate). Update `.env.example` to document the expected value (`https://<ref>.supabase.co/auth/v1`). Pairs naturally with the existing trustProxy gate — both share the "production must declare X" invariant. PR #82 round-1 security-reviewer LOW (`SUPABASE_JWT_ISSUER`).

  **Also fold in: CI/build pre-flight that fails if `NODE_ENV` is set as a Railway dashboard var (PR #83 security-reviewer LOW, new — diff-specific).** PR #83 moved `NODE_ENV=production` from Railway dashboard env into the inline `[deploy].startCommand`. The in-file comment says "REMOVE NODE_ENV from the Railway dashboard service vars before deploying" but this is documentation-only — nothing prevents a future operator from re-setting it on the dashboard, which would silently re-break the build (dashboard env applies to both build and runtime, so `NODE_ENV=production` there propagates to Nixpacks's implicit install and strips devDeps including `turbo`). The inline prefix in startCommand still wins at runtime regardless, so the failure mode is silent build breakage rather than a runtime regression — but a broken build means whatever last-known-good image was deployed gets re-deployed on the next push, masking the misconfiguration. **Fix shape**: add a CI pre-flight step (in `ci.yml` or a Railway build hook) that runs `railway variables list --service diktat-api | grep -E '^NODE_ENV='` and exits non-zero if any match — turn the documentation-only constraint into a mechanical gate. Either run it in CI (requires Railway CLI + a read-only token in GH secrets) OR add a Railway build hook (runs in the Railway build container with implicit auth). The CI form is more visible; the build hook form is closer to where the breakage happens. **Cross-reference**: this is conceptually paired with `check-activation-safety.sh` which mechanically enforces "ENABLE_RAILWAY_DEPLOY=true requires pinned workflows" — same shape ("flag-flip must not happen without its corresponding constraint landing atomically"), different layer. PR #83 round-1 security-reviewer LOW (diff-specific).

  **Also fold in: `WEB_ORIGINS` production assertion (PR #83 security-reviewer LOW, new — broader context).** `WEB_ORIGINS` defaults to `'http://localhost:3000'` in `env.ts`. If an operator forgets to set it on the Railway service env when deploying, the API silently rejects all cross-origin requests from the production web origin — CORS callback returns `cb(null, false)` for any origin not in the allow-list. Fail-closed from a security perspective, but masks a real deploy-time misconfiguration as a runtime "everything is mysteriously broken" symptom. **Fix shape**: extend the boot-time activation-safety gate in `server.ts` to assert that when `NODE_ENV !== 'development' && NODE_ENV !== 'test'`, `env.WEB_ORIGINS` does NOT contain `'localhost'` (or, more precisely: contains at least one origin that does NOT match `/^https?:\/\/localhost/`). Same gate shape as `TRUSTED_PROXY_HOPS` and the queued `SUPABASE_JWT_ISSUER` extension — all three are "production must declare X" invariants and can share the same `if (env.NODE_ENV !== 'development' && env.NODE_ENV !== 'test')` block. PR #83 round-1 security-reviewer LOW (`WEB_ORIGINS`).

  **Also fold in: `packages/auth/src/verify.ts` audience empty-string handling (PR #83 security-reviewer LOW, new — broader context).** `baseOpts` spreads `audience: opts.audience ?? 'authenticated'`. `??` coalesces only on `null | undefined` — a caller that explicitly passes `audience: ''` (empty string) bypasses the default and reaches `jose`'s `jwtVerify` with an empty audience expectation. `jose` then accepts tokens with ANY audience claim (or none) as valid. The current call sites all pass `audience` either as a non-empty string or via the default, so no exploit exists today, but the structural posture is wrong. **Fix shape**: add a runtime assertion inside `verifyJwt` that throws `ValidationError` if the resolved `audience` is an empty string after the `??` resolution. One-line guard; not load-bearing today, structurally hardens the function against future misconfigured call sites. PR #83 round-1 security-reviewer LOW (`audience` empty-string).

  **Also fold in: auth callback `url.origin` trusts the `Host` header (PR #83 security-reviewer LOW, new — broader context).** `apps/web/app/auth/{callback,confirm}/route.ts` construct redirect URLs via `new URL('/...', url.origin)`. In Next.js Route Handlers, `request.url` is constructed from the incoming `Host` header. If the deployment is ever placed behind a proxy that doesn't validate `Host`, a crafted request could redirect to an attacker-controlled origin. Supabase's `ADDITIONAL_REDIRECT_URLS` allow-list is the authoritative guard (Supabase validates the post-OAuth redirect before handing back a code), so today's exposure is bounded — but the app itself adds no secondary check. **Fix shape**: pin the redirect base to an explicit `NEXT_PUBLIC_SITE_URL` env var instead of deriving from `url.origin`. Belt-and-suspenders given the Supabase-side validation; eliminates the `Host`-header dependency at the app layer. Add `NEXT_PUBLIC_SITE_URL` to `.env.example` with the production Vercel domain as the example value. PR #83 round-1 security-reviewer LOW (auth callback `url.origin`).

  **Also fold in: move `outer_hook.resolved_ip` log inside the rate-limit `if (result.allowed)` branch (PR #84 security-reviewer LOW, new — diff-specific, deferred).** PR #84 shipped a one-line observability log in the Fastify `onRequest` hook that emits `{ event: 'outer_hook.resolved_ip', cidr, path }` BEFORE the rate-limit dispatch. A request that hits the IP-keyed 1200/min ceiling still consumes a log line on the way to its 429 response. Not a security issue — every 429'd request already emits the Fastify default `incoming request` + the response 429 — but it's redundant and adds log volume to the exact traffic pattern (sustained-burst-from-one-IP) where log compression matters most. **Fix shape**: move the `request.log.info(...)` call inside the `if (result.allowed) { ... }` branch in `server.ts` (currently around line 175, post-rate-limit-check). Note that this changes the observability semantic: a 429'd request will no longer emit the resolved-IP log, but the existing 429 response + `Retry-After` header + the structured `rate_limit.deny` log (when added) already cover that case. PR #84 round-1 security-reviewer LOW (pre-gate log placement). Deferred from PR #84 because the BLOCK-critical PII fix (raw-`ip` drop) was the only finding gating the PR; the pre-gate placement is a polish-grade observability concern best landed alongside the other process-hardening items.

  **Also fold in: `redact.paths` defensive add for `ip` + `req.remoteAddress` (PR #84 security-reviewer LOW, new — diff-specific, deferred).** The existing Pino `redact.paths` in `apps/api/src/server.ts:99-105` covers `req.headers.{authorization,cookie}` + three `err.cause.*` fields. It does NOT cover the `ip` key on arbitrary log objects, nor `req.remoteAddress`. PR #84's BLOCK was closed by removing the raw `ip` from the new outer-hook log line — but Pino's `redact` is path-based and operates on the serialized object shape, so a future accidental `request.log.info({ ip: request.ip, ... })` anywhere in the codebase would NOT be caught by the redact list. **Fix shape**: extend the `redact.paths` array in the Fastify constructor to also include `'ip'` and `'req.remoteAddress'`. Defensive-only — no current log line emits these — but it's the structural backstop that prevents future regressions of the PR #84 round-1 BLOCK. Note: `req.remoteAddress` IS currently emitted by Fastify's default request logger; adding it to redact would change observable log shape on Vercel/Railway/Axiom sinks. Audit whether any operator-facing dashboard depends on that field before flipping. Pure defense-in-depth; trivially safe to add. PR #84 round-1 security-reviewer LOW (redact list coverage). Deferred from PR #84 for the same reason as the placement LOW above.

  **Bundled rationale**: all ten findings target the pre-Railway-activation surface. Four layers:
  - **Build/runtime image hygiene** — strip devDeps from runtime container (MED #2); strip absolute source paths from shipping `.d.ts.map` artifacts (MED #1).
  - **Boot-time activation gates extension** — CORS null-origin (MED); `SUPABASE_JWT_ISSUER` required in prod (LOW); `WEB_ORIGINS` production assertion (LOW). The three boot-gate extensions naturally pair with the existing `TRUSTED_PROXY_HOPS` gate from PR #78 (same "production must declare X" pattern, same `if (NODE_ENV !== 'development' && NODE_ENV !== 'test')` block).
  - **Process hardening** — Railway dashboard `NODE_ENV` CI preflight (LOW); `verify.ts` empty-string `audience` assertion (LOW); auth-callback redirect base via `NEXT_PUBLIC_SITE_URL` not `Host`-header-derived `url.origin` (LOW).
  - **Observability hygiene** — move `outer_hook.resolved_ip` log inside `if (result.allowed)` branch (LOW); extend Pino `redact.paths` with `ip` + `req.remoteAddress` for defense-in-depth (LOW).

  Same review cycle, same "before Railway activation" timeline. Nine of the ten edits are trivially safe (the install-mode shift in MED #2 is the only one that needs its own local-verify pass before pushing — proven once already). Sized to land as one focused follow-up PR. Not exploitable under the current pre-production posture (deploy flags unset, no public DNS, API not reachable). Deferred from PRs #80 / #82 / #83 / #84 because the security-reviewer's literal `pnpm prune --prod` recommendation needed correction and that correction wanted its own reviewer cycle rather than gating the verified Railway-build-unblock fixes; the CORS / JWT_ISSUER / boot-gate-extension / verify-audience / auth-callback / NODE_ENV-preflight findings joined the bundle on PRs #82 and #83's rounds (Mode B-adjacent broader-context findings on PR #82, explicitly-labeled diff-and-context findings on PR #83); the two PR #84 observability LOWs joined after the BLOCK-critical PII fix shipped in PR #84 itself — all real, all queued.
