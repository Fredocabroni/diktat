...
- **[M5.1 — recon first] High-frequency polling-query rate limits.** PR #56 round-1 security-reviewer (HIGH/MEDIUM) flagged four unrated query/polling procedures: `battles.getRound` (the 1-second polling endpoint while a battle is live), `battles.getBattle`, `debates.getBattle` (5-query fan-out per call), `matchmaking.getStatus` (polled every second while queueing). Plus four cold reads: `user.me`, `feed.list`, `factCheck.getVerdict`, `pushSubscriptions.listMine`. **Reviewer-proposed budgets**: `getRound` 120/min, `getBattle` 60/min, `debates.getBattle` 30/min, `getStatus` 120/min, `user.me` 60/min, `feed.list` 120/min, `getVerdict` 30/min, `listMine` 20/min. The signed-off M5 design DEFERRED per-procedure caps on read queries to a Phase-4 follow-up (the outer hook is the floor); the reviewer's argument is that high-frequency polling at >60 req/s by one authenticated session is the abuse vector that deferral implicitly allowed. **Reverses a signed-off design choice — recon FIRST**: instrument or grep the actual client polling cadences (`apps/web/app/(app)/battles/[battleId]/page.tsx`, `apps/web/app/(app)/matchmaking/page.tsx`, the `useInfiniteQuery` polling intervals in `wallet/page.tsx`, etc.) BEFORE picking numbers. A 1Hz client polling endpoint with `2× safety margin = 120/min` is the reviewer's suggestion; the real cadence may differ. Build under M5.1 with the same factory pattern. **PR #56 round-3 reviewer escalation**: promote M5.1 to `[P0-before-prod]` specifically because `debates.getBattle` runs a 5-query parallel fan-out per call. A single authenticated session hammering it at >60 req/s exhausts the Supabase connection pool before the outer 1200/min IP ceiling fires. Promote the entry timeline to "before the first public-DNS exposure" (same gate as the trustProxy fix above), not just "before public launch."
