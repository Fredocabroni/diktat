...
- **[P0-with-deploy-activation] Deploy-workflow hardening bundle — `deploy-railway.yml` + `deploy-vercel.yml`.** PR #71 round-1 reviewer raised both as HIGH; round-2 reviewer escalated the verdict to BLOCK with the structural objection _"a queue entry is not a CI gate"_. The user explicitly overrode the BLOCK to merge PR #71 on the documented gate shape below — this entry is the authoritative ship plan for the override. Both workflows are no-op today via `ENABLE_RAILWAY_DEPLOY=false` / `ENABLE_VERCEL_DEPLOY=false` — dormant risk. **Hard gate (atomic ship)**: this bundle MUST land in the same PR that flips either `ENABLE_*=true`, with BOTH the pins below AND the activation-safety assertion in (4) — neither half is sufficient alone. Same hard-gate status as the `trustProxy` boot assertion. Four surfaces in one sweep:
...
- **[MED, reviewer-workflows npm install pin] `@anthropic-ai/claude-code` installed without version pin in `claude-review.yml` + `claude-migrate-review.yml`.** PR #71 round-2 reviewer MED (PR #68 territory). Both reviewer workflows currently invoke `npm i -g @anthropic-ai/claude-code` with no version constraint. The next step runs the agent with `ANTHROPIC_API_KEY` in env. An unexpected breaking change OR a compromised publish to the `latest` dist-tag would silently alter reviewer behavior or exfiltrate the API key — the agent step is the highest-privilege step in these workflows. **Fix shape**: pin to `@anthropic-ai/claude-code@<exact-version>` in both workflows; add Renovate/Dependabot grouping rule so future bumps land as a single coordinated PR with reviewer-output regression checks rather than a silent npm-registry resolution. Not a blocker for non-deploy work but should land in the next CI hygiene bundle.
