...
- **Audit `apps/workers/src/jobs/battle-runner.ts` (trivia runner) for the same scoreAndSettle crash-window shape addressed by the open-debate resume guard.** If it has the same verdict-write → AP-apply → mark-settled sequence with no resume branch on partial completion, the same fix applies in its own PR: stamp `_settlement_inputs` on the verdict write, dispatch the in-flight state to a `resumeSettlement` branch, add `resumed_settlement` TickPhase + structured log. Out of scope for the open-debate PR — separate audit + separate fix.
