...
- **[P0-before-prod] M5 `trustProxy` gate — Railway deploy activation MUST ship `trustProxy` in the same commit.** Fastify's `request.ip` returns the immediate TCP peer when `trustProxy` is unset. The api ships today with `trustProxy` NOT set — appropriate for local dev (no proxy). The moment Railway lights up (`deploy-railway.yml` is currently a no-op stub gated on `ENABLE_RAILWAY_DEPLOY=true`), every IP-keyed counter in `publicLimit` and `checkGlobalOuterHook` collapses to a single Redis key keyed by the Railway edge IP. The outer-hook `1200/min` ceiling becomes a 1200/min ceiling on the WHOLE service. The `auth.session 600/min IP /24` budget and `tribes.list 300/min IP /24` budget are fully bypassed. **Surfaced live by PR #56 round-1 security-reviewer (HIGH).** **Hard gate**: the same PR that flips `ENABLE_RAILWAY_DEPLOY=true` MUST add `trustProxy: <real-hop-count>` to the Fastify constructor in `apps/api/src/server.ts:11`. The hop count is whatever Railway's `X-Forwarded-For` chain depth turns out to be — typically `1` for direct Railway, more if Cloudflare or another CDN sits in front. Confirm empirically before merging by curling `/health` and reading `request.ip` from the Fastify log; if it shows the public client IP, the hop count is correct. **PR #56 round-3 reviewer added a code-gate recommendation** (not just a doc-gate): add a startup assertion in `apps/api/src/server.ts` boot that throws when `env.ENABLE_RAILWAY_DEPLOY === 'true' && !fastifyOptions.trustProxy`. The assertion is the belt-and-suspenders backstop against a future PR that flips the flag without setting trustProxy — fail-fast in the boot path rather than silently collapsing every IP-keyed counter to the proxy IP. Land the assertion in the same commit as the trustProxy setting. **Bundle the public-tier IP-safety work as one unit** — same pre-public-DNS timeline, same fix surface (hybrid IP+userId keying). The two co-NAT findings in the M5 round-3 leftovers entry below (#2 INCR-then-check counter inflation + #6 co-NAT denial-of-service) share this gate's deploy boundary: an authenticated user behind a busy CGNAT /24 should never be 429'd by a co-NAT abuser's burst, and a burst client must not be able to inflate the shared counter against co-NAT victims. The recommended fix at both ends is the same — hybrid keying: when `ctx.userId` is null (anonymous traffic, e.g. pre-sign-in `auth.session` polling), IP-key; when present, user-key. That single change closes BOTH co-NAT vectors AND aligns the deploy gate: trustProxy + hybrid keying + the startup assertion all land together before `ENABLE_RAILWAY_DEPLOY=true` is committed. **Do NOT** treat the three findings as separate items to schedule independently — the public-tier IP-safety surface is one bundle.
...
- **[M5 round-3 leftovers] PR #56 round-3 security-reviewer findings — eight items deferred from the final pre-merge round.** Per the user's standing instruction ("any findings a round-3 reviewer raises on round-3's own changes get queued, not chased — M5 merges after this"). None are merge-blocking; sized to land in a single small follow-up PR or fold into M5.1.
  ...
  2. **[MEDIUM] `auth.session` INCR-then-check enables co-NAT counter inflation.** `SINGLE_GATE_LUA` increments before the limit check, so a denied call at limit+1 pushes the counter to limit+2. For `auth.session` (publicLimit, IP-keyed /24 CIDR) this means an aggressive client in a CGNAT pool can inflate the shared counter and 429 every other user behind the same NAT. **Fix shape**: hybrid keying — when `ctx.userId` is null, IP-key; when present, user-key — closes this AND the co-NAT denial below (#6) in one change. **BUNDLED with the trustProxy P0-before-prod gate above**: same pre-public-DNS timeline, same code surface (`auth.session` is the only IP-keyed authed-eligible procedure), one PR ships trustProxy + startup assertion + hybrid keying together.
  ...
  6. **[LOW] `auth.session` co-NAT denial-of-service vector.** Same root as #2 but from the victim direction: one aggressive client in a /24 can exhaust the 600/min budget and 429 every other user behind that NAT on page-load. **Mitigation**: hybrid IP+userId keying — authenticated users get user-keyed counters, anonymous traffic gets IP-keyed. **BUNDLED with the trustProxy P0-before-prod gate above and #2 in this entry**: see the trustProxy entry for the unified ship plan. Tracked separately as LOW because the victim-direction framing matters for the queue's prioritization audit, but the FIX is one change covering #2 + #6 + the trustProxy gate.
...
- **[M5.1 round-3 leftovers] PR #62 round-3 security-reviewer findings — five items queued from the terminal round.** Per the standing instruction: round-3 is terminal, new findings get queued unless they are real blocking defects in the wallet wiring. None landed against the wallet wiring this PR shipped; everything below is either a pre-existing finding the reviewer re-surfaced under a new cumulative-PR scope or a new advisory.
  ...
  4. **[LOW] procedure-arg allowlist regex on `queryLimit` / `mutationLimit` + `actions/checkout@v4` SHA pin sweep + extend allowlist to `publicLimit` / `aiSpendLimit`.** Hardening items, bundle in one PR. (a) ~~`queryLimit(procedure, ...)` and `mutationLimit(procedure, ...)` embed `procedure` directly into the Redis key with no runtime assertion.~~ **CLOSED in PR #67 commit `ef0186f`** — `PROCEDURE_RE = /^[a-zA-Z][a-zA-Z0-9._-]*$/` + `assertProcedureSlug(factory, slug)` helper now throw at module-load when `queryLimit` or `mutationLimit` is wired with a malformed slug. PR #62 round-2 LOW-3. (b) `.github/workflows/claude-review.yml` and `claude-migrate-review.yml` still use `actions/checkout@v4` and `actions/setup-node@v4` as floating tags; the new `migrations.yml` also uses `actions/checkout@v4` (PR #62 round-2 MEDIUM-3 — load-bearing for migration correctness). Pin all three sites to the v4.2.2 SHA `11bd71901bbe5b1630ceea73d27597364c9af683`. (`setup-cli`'s `version: latest → 2.1.1` ships in a separate one-line PR — see below — and is not part of this bundle.) (c) **`publicLimit` + `aiSpendLimit` allowlist extension** — PR #67 round-1 reviewer LOW-2 + INFO-1. The PROCEDURE_RE guard ships on `queryLimit` + `mutationLimit` only (deliberate scope per the disclosure-pass user instruction). `publicLimit(procedure, ...)` (`apps/api/src/rate-limit.ts:494`) and `aiSpendLimit(procedure, ...)` (`apps/api/src/rate-limit.ts:251`) embed `procedure` into Redis keys the same way and have no equivalent guard. Currently both factories have hard-coded literal call sites (`auth.session`, `tribes.list`, `factCheck.enqueue`) so neither is exploitable today, but the pattern is now asymmetric — a developer would reasonably assume the guard covers all four factories. **Fix shape**: one line in each factory body — `assertProcedureSlug('publicLimit', procedure)` + `assertProcedureSlug('aiSpendLimit', procedure)`. Pair with this PR's same `assertProcedureSlug` helper, no new code. The asymmetry is deliberate, not an oversight — revisit when extending the guard to the public/aiSpend tiers in this bundle's eventual landing PR.
