...
- **[M5.1 round-3 leftovers] PR #62 round-3 security-reviewer findings — five items queued from the terminal round.** Per the standing instruction: round-3 is terminal, new findings get queued unless they are real blocking defects in the wallet wiring. None landed against the wallet wiring this PR shipped; everything below is either a pre-existing finding the reviewer re-surfaced under a new cumulative-PR scope or a new advisory.
  ...
  2. **[security-disclosure pass — bundle] `debates.castVote` `apWeight` wire leak + `battle_participants` RLS handle disclosure.** Two findings, one disclosure-review pass. (a) `apps/api/src/routers/debates.ts:323` — `castVote` returns `{ ok, voteId, apWeight: voterRow.current_ap }`. The `apWeight` field is unnecessary on the wire and acts as an oracle for the server's authoritative AP balance every time the user votes. **Fix shape**: drop `apWeight` from the return; if the UI needs the snapshot, it reads `user.me`. PR #62 round-3 MEDIUM-2. (b) `apps/api/src/routers/debates.ts:68-69` — `debates.getBattle` joins `users(handle)` onto `battle_participants` and any authenticated user can call it with any `battleId` UUID and enumerate participant handles. Depends on RLS posture on `battle_participants` — confirm intent, then either strip the handle field pre-settlement or document the spectator-visibility contract explicitly. PR #62 round-2 LOW-2.
  ...
  4. **[LOW] procedure-arg allowlist regex on `queryLimit` / `mutationLimit` + `actions/checkout@v4` SHA pin sweep.** Two cheap hardening items, bundle in one PR. (a) `queryLimit(procedure, ...)` and `mutationLimit(procedure, ...)` embed `procedure` directly into the Redis key with no runtime assertion. All current call sites pass hard-coded literals so it is not currently exploitable, but a startup-time guard (`PROCEDURE_RE = /^[a-zA-Z][a-zA-Z0-9._-]*$/`; throw on mismatch) closes the analogous injection vector that `subjectSchema.parse()` UUID validation closes for `userId`. PR #62 round-2 LOW-3. (b) `.github/workflows/claude-review.yml` and `claude-migrate-review.yml` still use `actions/checkout@v4` and `actions/setup-node@v4` as floating tags; the new `migrations.yml` also uses `actions/checkout@v4` (PR #62 round-2 MEDIUM-3 — load-bearing for migration correctness). Pin all three sites to the v4.2.2 SHA `11bd71901bbe5b1630ceea73d27597364c9af683`. (`setup-cli`'s `version: latest → 2.1.1` ships in a separate one-line PR — see below — and is not part of this bundle.)
