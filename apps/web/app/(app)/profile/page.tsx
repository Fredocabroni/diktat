...
export default function ProfilePage() {
  ...
  return (
    <section className="mx-auto flex max-w-md flex-col items-center px-4 py-8">
      ...
      <h1 className="mt-5 font-display text-3xl font-bold tracking-tight text-text-primary">
        {displayName}
      </h1>
      <button
        type="button"
        onClick={() => setEditing(true)}
        className="mt-1 text-sm text-text-secondary underline-offset-4 hover:text-text-primary hover:underline"
      >
        @{me.handle}
      </button>
      ...
      <dl className="mt-8 grid w-full grid-cols-3 gap-3 text-center">
        <div className="rounded-2xl border border-ink-300 bg-surface-card p-4">
          ...
          <dd className="mt-1 font-display text-2xl font-bold text-text-primary">
            {me.current_ap}
          </dd>
        </div>
        <div className="rounded-2xl border border-ink-300 bg-surface-card p-4">
